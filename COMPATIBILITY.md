...
| Share URLs | Shared payloads accept only options known to the current Spectre schema and validate their shape. Unknown keys are intentionally dropped at this trust boundary, so share URLs are not a lossless transport for future or nightly Ghostty options. |
